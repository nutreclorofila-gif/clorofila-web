...
Use the exact same `<head>`/nav/footer/script scaffolding as Task 6's file (adjusting only the page-specific values below), with this content:

- `<title>`: `¿Qué le pasa al omega-3 del lino cuando horneás? — Clorofila`
- meta description / og:description: `El ALA del lino es el ácido graso más inestable al calor. Qué le pasa cuando horneás a temperaturas reales, y qué podés hacer al respecto.`
- canonical / og:url: `https://clorofila.uy/articulos/omega-3-lino-horneado`
- JSON-LD `Article` headline: `¿Qué le pasa al omega-3 del lino cuando horneás?`, `mainEntityOfPage`: `https://clorofila.uy/articulos/omega-3-lino-horneado`
- JSON-LD `BreadcrumbList` position 3: `name: "Omega-3 del lino al horno"`, `item: "https://clorofila.uy/articulos/omega-3-lino-horneado"`
- Breadcrumb visual text: `Omega-3 del lino al horno`
- `section-label`: `Omega-3`
- `<h1>`: `¿Qué le pasa al omega-3 del lino cuando horneás?`
- Related articles list: link to `aceite-de-oliva-punto-de-humo.html` ("Aceite de oliva y punto de humo →"), `lavado-de-frutas-bicarbonato.html` ("Lavado de frutas: bicarbonato vs agua sola →"), `mas-alla-del-colesterol-total.html` ("Más allá del colesterol total →")

Body content (inside `<article class="articulo-body">`, before the `.articulo-cta` div):
...
Same scaffolding as Task 6, with:

- `<title>`: `Lavado de frutas: bicarbonato vs agua sola — Clorofila`
- meta description / og:description: `El bicarbonato ayuda con residuos de contacto en la cáscara. Para plaguicidas sistémicos, ningún lavado alcanza. La distinción importa para saber qué esperar.`
- canonical / og:url: `https://clorofila.uy/articulos/lavado-de-frutas-bicarbonato`
- JSON-LD `Article` headline: `Lavado de frutas: bicarbonato vs agua sola`, `mainEntityOfPage`: `https://clorofila.uy/articulos/lavado-de-frutas-bicarbonato`
- JSON-LD `BreadcrumbList` position 3: `name: "Lavado de frutas con bicarbonato"`, `item: "https://clorofila.uy/articulos/lavado-de-frutas-bicarbonato"`
- Breadcrumb visual text: `Lavado de frutas con bicarbonato`
- `section-label`: `Frutas y verduras`
- `<h1>`: `Lavado de frutas: bicarbonato vs agua sola`
- Related articles: `aceite-de-oliva-punto-de-humo.html` ("Aceite de oliva y punto de humo →"), `omega-3-lino-horneado.html` ("¿Qué le pasa al omega-3 del lino cuando horneás? →"), `mas-alla-del-colesterol-total.html` ("Más allá del colesterol total →")
...
- `<title>`: `Más allá del colesterol total — Clorofila`
- meta description / og:description: `Apo-B, tamaño de partícula LDL y otros marcadores dan una imagen más completa que el colesterol total solo. Qué significan y por qué cambian el panorama.`
- canonical / og:url: `https://clorofila.uy/articulos/mas-alla-del-colesterol-total`
- JSON-LD `Article` headline: `Más allá del colesterol total`, `mainEntityOfPage`: `https://clorofila.uy/articulos/mas-alla-del-colesterol-total`
- JSON-LD `BreadcrumbList` position 3: `name: "Más allá del colesterol total"`, `item: "https://clorofila.uy/articulos/mas-alla-del-colesterol-total"`
- Breadcrumb visual text: `Más allá del colesterol total`
- `section-label`: `Grasas y salud`
- `<h1>`: `Más allá del colesterol total`
- Related articles: `aceite-de-oliva-punto-de-humo.html` ("Aceite de oliva y punto de humo →"), `omega-3-lino-horneado.html` ("¿Qué le pasa al omega-3 del lino cuando horneás? →"), `lavado-de-frutas-bicarbonato.html` ("Lavado de frutas: bicarbonato vs agua sola →")
